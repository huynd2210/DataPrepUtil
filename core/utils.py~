...
def compareSetsWithDiff(set1, set2):
    if set1 == set2:
        return {
            "equal": True,
            "diff_in_list1": {},
...
def isDataFrameEqual(df1, df2):
    ...
    for i in range(len(df1Values)):
        comparison = compareCountersWithDiff(Counter(df1Values[i]), Counter(df2Values[i]))
        # comparison = compareSetsWithDiff(set(df1Values[i]), set(df2Values[i]))

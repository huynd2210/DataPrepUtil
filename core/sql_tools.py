...
def print_schema(schema: DatabaseSchema):
    """
    Pretty print the schema information.

    Args:
        schema (DatabaseSchema): Schema information from inspect_database()
    """
    for table in schema.tables:
        print(f"\nTable: {table.name}")
        print("-" * (len(table.name) + 7))
        ...
        for col in table.columns:
            ...
            print(f"{pk_marker}{col.name} ({col.type}) {nullable_marker} {default}".strip())
